...
const CONVERSATION_MOVES = new Set(['follow-thread','clarify','deepen','challenge','offer-choice','transition']);
...
export default async function handler(req:Req,res:Res){
  ...
  const rawPerformance=body.performanceSnapshot&&typeof body.performanceSnapshot==='object'?body.performanceSnapshot:{};
  const performanceSnapshot={fluency:Number.isFinite(Number(rawPerformance.fluency))?Math.max(0,Math.min(100,Math.round(Number(rawPerformance.fluency)))):undefined,grammar:Number.isFinite(Number(rawPerformance.grammar))?Math.max(0,Math.min(100,Math.round(Number(rawPerformance.grammar)))):undefined,vocabulary:Number.isFinite(Number(rawPerformance.vocabulary))?Math.max(0,Math.min(100,Math.round(Number(rawPerformance.vocabulary)))):undefined,relevance:Number.isFinite(Number(rawPerformance.relevance))?Math.max(0,Math.min(100,Math.round(Number(rawPerformance.relevance)))):undefined,average:Number.isFinite(Number(rawPerformance.average))?Math.max(0,Math.min(100,Math.round(Number(rawPerformance.average)))):undefined,trend:['improving','stable','struggling'].includes(rawPerformance.trend)?rawPerformance.trend:undefined};
  ...
  try{
    ...
    const reply:any={
      ...
      conversationMemory:parsed.conversationMemory&&typeof parsed.conversationMemory==='object'?{
        ...
        conversationMove:CONVERSATION_MOVES.has(parsed.conversationMemory.conversationMove)?parsed.conversationMemory.conversationMove:undefined,
