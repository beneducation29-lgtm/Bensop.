...
export default async function handler(req:Req,res:Res){
  ...
  try{
    ...
    res.status(200).json({content:String(parsed.content||''),suggestions:Array.isArray(parsed.suggestions)?parsed.suggestions.map(String):[],action:parsed.action&&typeof parsed.action==='object'?{label:String(parsed.action.label||'OPEN NEXT STEP'),path:String(parsed.action.path||'/adaptive-session')}:undefined,source:'ai'});
  }catch{
