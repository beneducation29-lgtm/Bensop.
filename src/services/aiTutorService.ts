...
class AITutorService{
  getLearnerContext(language:'en'|'zh'='en'):TutorContext{
    ...
    const weakest=profile.needsAttention[0];
    return{language,level:'B1',learner:{
      overall:profile.overall,weakestSkill:weakest?.key,weakestScore:weakest?.score,
      weakestTrend:weakest?.trend,dueReviews:spacedReviewService.getDue().length,
      recentEvidenceCount:profile.recentEvidence.length,
    }};
  }
  ...
  async reply(message:string,context:TutorContext):Promise<TutorReply>{
    ...
    const weakest=learner?.weakestSkill;
    const weakLabel=weakest?skillLabels[weakest]?.[zh?'zh':'en']:undefined;
    ...
    if(q.includes('grammar')||q.includes('ngữ pháp')||q.includes('语法')){
      ...
      if(weakLabel){
        content=zh?'根据你的学习记录，建议先加强'+weakLabel+'。当前记录约为'+(learner?.weakestScore??0)+'%。'+(learner?.weakestTrend==='down'?'最近趋势有所下降。':'先做一个短练习即可继续积累证据。'):
    ...
    return{content,suggestions,source:'fallback'};
  }
}
