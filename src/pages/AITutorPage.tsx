...
import{Bot,Send,Target,RefreshCw,ArrowRight}from'lucide-react';
...
export function AITutorPage({onNavigate}:{onNavigate?:(path:string)=>void}){
  ...
  const[messages,setMessages]=useState<Array<TutorMessage&TutorReply>>([]),[input,setInput]=useState(''),[busy,setBusy]=useState(false);
  const send=async(text=input.trim())=>{
    if(!text||busy)return; setInput('');
    setMessages(m=>[...m,{id:String(Date.now()),role:'user',content:text,createdAt:new Date().toISOString(),suggestions:[],source:'fallback'}]);
    setBusy(true); const r=await aiTutorService.reply(text,context);
    setMessages(m=>[...m,{id:String(Date.now()+1),role:'assistant',content:r.content,createdAt:new Date().toISOString(),...r}]); setBusy(false);
  };
